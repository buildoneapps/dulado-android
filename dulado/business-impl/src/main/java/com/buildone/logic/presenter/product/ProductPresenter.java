...
public class ProductPresenter implements ProductContract.Presenter {
    ...
    public void start() {
        ...
        if(product.getId() > 0){
            ...
            product = new ProductObject(productSearch.getId(),
                    productSearch.getSeller().getId(),
                    productSearch.getPrice(),
                    productSearch.getProductName(),
                    "Os objetos utilizados nas fotos são exatamente os mesmos produtos vendidos. Deliciosos cookies de creme de avelã",
                    new ArrayList<String>(){{add(productSearch.getImageUrl());}},
                    productSearch.getSeller(), productSearch.getTag(), 10);
            setupProduct(product);
        }
    }
    ...
    public void setupProduct(ProductObject product) {
        ...
        view.setProductTags(product.getTags());
